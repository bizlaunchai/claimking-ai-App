...
/**
 * Conversation the user is looking at right now, if any. Its unread messages
 * are excluded server-side from the badge count: they're being read the moment
 * they arrive, so counting them would show a number that clears itself a beat
 * later. The Messages page sets this on open and clears it on close — and also
 * clears it when the tab goes to the background, because then nobody IS
 * reading and the badge should count normally again.
 */
    ...
    inFlight = (async () => {
        try {
            const { data } = await axiosInstance.get('/messages/unread-count', {
                params: activeThreadId ? { exclude_thread: activeThreadId } : {},
...
/**
 * Tell the badge which conversation is on screen so it stops counting it.
 * Pass `null` when the conversation closes or the tab loses focus.
 */
export function setActiveMessageThread(clientPortalId) {
    const next = clientPortalId || null;
    if (activeThreadId === next) return;
    activeThreadId = next;
    // A request may already be in flight with the OLD exclusion — it would land
    // after us and overwrite the corrected count, so refetch once it settles.
    if (inFlight) {
        inFlight.then(() => fetchCount());
        return;
    }
    fetchCount();
}
