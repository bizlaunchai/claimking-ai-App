...
export async function updateSession(request) {
  ...
  if (user && !isPublicPath(pathname) && pathname.startsWith("/dashboard")) {
    try {
      ...
      // ── 2a. Superadmin-only paths ──────────────────────────────────
      // /dashboard/admin/* is for ClaimKing platform owners. Company users
      // — even company admins — get bounced to their own dashboard.
      if (isSuperadminPath(pathname) && !isSuperadmin) {
        const url = request.nextUrl.clone();
      ...
      // Plain team members must already have a company. Superadmin bypasses.
