...
// Auth pages an ALREADY-authenticated user should never see — they have a
// session, so bounce them to the dashboard. /auth/update-password is NOT here:
// it's reached via a password-recovery link where the user is signed in with a
// short-lived recovery session and legitimately needs the page.
const AUTH_PAGES_BLOCKED_WHEN_LOGGED_IN = [
  "/auth/login",
  "/auth/sign-up",
...
export async function updateSession(request) {
  ...
  if (
    ...
    // /plans is the public pricing page (the header "Start Free Trial" target).
    // Guests browse plans there, then sign up.
    ...
    // /book/* is the public per-rep appointment booking page. The slug is the
    // only credential; homeowners aren't logged in. Backend enforces validity,
    // rate-limit, and captcha.
    !pathname.startsWith("/book") &&
    // /sub/* is the public subcontractor portal + onboarding wizard. The
    // portal_token is the only credential (subs aren't dashboard users). Same
    // model as /portal and /book — backend resolves + enforces the token.
    !pathname.startsWith("/sub/") &&
    // /unsubscribe is the public CAN-SPAM one-click unsubscribe page (token-based).
    !pathname.startsWith("/unsubscribe") &&
    // /pay/* is the homeowner-facing deposit payment result page (Stripe
    // success/cancel redirect). Homeowners aren't logged in — no auth needed.
  ...
  // ── 1b. Authenticated → keep them OFF login / signup / forgot-password ──
  // They already have a session; bounce to the dashboard. Exception: a
  // suspension redirect (auth_notice cookie) is let through so the login page
  // can show the message and sign them out — otherwise we'd loop.
  if (
    user &&
    AUTH_PAGES_BLOCKED_WHEN_LOGGED_IN.some((p) => pathname.startsWith(p)) &&
    !request.cookies.get("auth_notice")
  ) {
    const url = request.nextUrl.clone();
  ...
  // ── 1c. Authenticated WITH a plan → keep them OFF the plan pages ───────
  // A user with a company_id has already bought a plan; the public /plans
  // pricing page and the /onboarding picker are meaningless to them, so bounce
  // to the dashboard. Users without a company (need to pick a plan) and
  // platform users still get through.
  if (user && (pathname.startsWith("/onboarding") || pathname.startsWith("/plans"))) {
    try {
      ...
      // Profile lookup failed — fall through; the page handles its own state.
    }
  }
  ...
  if (pathname.startsWith("/portal") || pathname.startsWith("/sign") || pathname.startsWith("/pay") || pathname.startsWith("/sub/")) {
  ...
  if (user && !isPublicPath(pathname) && pathname.startsWith("/dashboard")) {
    try {
      ...
      // Redirect to login with a one-shot flash cookie (keeps the URL clean —
      // no ?error=... query string). The login form reads + clears it.
      const redirectWithNotice = (notice) => {
        const url = request.nextUrl.clone();
        url.pathname = "/auth/login";
        url.search = "";
        const res = NextResponse.redirect(url);
        res.cookies.set("auth_notice", notice, {
          path: "/",
          maxAge: 30,
          sameSite: "lax",
        });
      ...
      if (profile?.status === "suspended") {
        return redirectWithNotice("account_suspended");
      }

      // Company-level suspension → block every member (owner + team), even
      // though their individual profile is still active.
      if (profile?.company_id) {
        ...
        if (company?.status === "suspended") {
          return redirectWithNotice("company_suspended");
        }
      }
